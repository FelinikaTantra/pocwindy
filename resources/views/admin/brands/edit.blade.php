...
<div class="bg-white rounded-lg shadow-sm p-6 max-w-lg">
    <h2 class="text-xl font-bold text-gray-800 mb-6">Edit Brand</h2>
    ...
    <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            @if($brand->logo)
                <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" class="h-16 w-16 object-contain mb-2 rounded border">
            @endif
